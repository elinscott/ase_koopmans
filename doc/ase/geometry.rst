...
* To get the actual value of a bond/angle/dihedral use ``instance.get_xxx_value(tuple)``. Note that you must pass ``mic=True`` to these calls if you're using PBCs and want the minimum image condition to be used for the calculation the values (see :class:`~ase.Atoms`).
* To get a lot of bond/angle/dihedral values at once use :func:`Analysis.get_values()`. Note the requirement of passing ``mic=True`` to obtain correct results when using PBCs, see previous point. 
