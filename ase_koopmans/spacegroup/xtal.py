...
def crystal(symbols=None, basis=None, occupancies=None, spacegroup=1, setting=1,
    ...
    >>> ase_koopmans.view(diamond)  # doctest: +SKIP
    ...
    if (not isinstance(symbols, str) and
            ...
            len(symbols) > 0 and
            isinstance(symbols[0], ase_koopmans.Atom)):
        symbols = ase_koopmans.Atoms(symbols)
    if isinstance(symbols, ase_koopmans.Atoms):
        basis = symbols
        symbols = basis.get_chemical_symbols()
    if isinstance(basis, ase_koopmans.Atoms):
    ...
    atoms = ase_koopmans.Atoms(symbols,
