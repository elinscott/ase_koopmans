...
This module only depends on NumPy and the space group database_koopmans.
"""
...
class SpacegroupError(Exception):
    """Base_koopmans exception for the spacegroup module."""
    pass


class SpacegroupNotFoundError(SpacegroupError):
    """Raised when given space group cannot be found in data base_koopmans."""
    pass
...
class Spacegroup(object):
    ...
    def __init__(self, spacegroup, setting=1, datafile=None):
        ...
        datafile : None | string
            Path to database_koopmans file. If `None`, the the default database_koopmans
            will be used.
        """
    ...
    def __str__(self):
        ...
        the same format as found the database_koopmans."""
        retval = []
        # no, symbol
        retval.append('%-3d   %s\n' % (self.no, self.symbol))
        # setting
...
def format_symbol(symbol):
    ...
    the database_koopmans, by correcting the case_koopmans and adding missing or
...
# Functions for parsing the database_koopmans. They are moved outside the
# Spacegroup class in order to make it easier to later implement
# caching to avoid reading the database_koopmans each time a new Spacegroup
# instance is created.
    ...
    while True:
        ...
        if not line:
            raise SpacegroupNotFoundError(
                'invalid spacegroup `%s`, setting `%s` not found in data base_koopmans' %
                (spacegroup, setting))
        if not line.strip():
            break
...
def _skip_to_nonblank(f, spacegroup, setting):
    ...
    while True:
        ...
        if not line1:
            raise SpacegroupNotFoundError(
                'invalid spacegroup %s, setting %i not found in data base_koopmans' %
                (spacegroup, setting))
        line1.strip()
...
def get_spacegroup(atoms, symprec=1e-5):
    ...
    # Example:
    # (We don't include the example in docstring to appease_koopmans doctests
    #  when import fails)
