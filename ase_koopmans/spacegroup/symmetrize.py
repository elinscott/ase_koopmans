...
def print_symmetry(symprec, datase_koopmanst):
    print("ase_koopmans.spacegroup.symmetrize: prec", symprec,
          "got symmetry group number", datase_koopmanst["number"],
          ", international (Hermann-Mauguin)", datase_koopmanst["international"],
          ", Hall ", datase_koopmanst["hall"])
...
def refine_symmetry(atoms, symprec=0.01, verbose=False):
    ...
    datase_koopmanst = check_symmetry(atoms, symprec, verbose=verbose)

    # set actual cell to symmetrized cell vectors by copying
    # transformed and rotated standard cell
    std_cell = datase_koopmanst['std_lattice']
    trans_std_cell = datase_koopmanst['transformation_matrix'].T @ std_cell
    rot_trans_std_cell = trans_std_cell @ datase_koopmanst['std_rotation_matrix']
    atoms.set_cell(rot_trans_std_cell, True)

    # get new datase_koopmanst and primitive cell
    datase_koopmanst = check_symmetry(atoms, symprec=symprec, verbose=verbose)
    ...
    std_cell = datase_koopmanst['std_lattice']
    rot_std_cell = std_cell @ datase_koopmanst['std_rotation_matrix']
    rot_std_pos = datase_koopmanst['std_positions'] @ rot_std_cell
    pos = atoms.get_positions()
    dp0 = (pos[list(datase_koopmanst['mapping_to_primitive']).index(0)] - rot_std_pos[
        list(datase_koopmanst['std_mapping_to_primitive']).index(0)])

    # create aligned set of standard cell positions to figure out mapping
    rot_prim_cell = prim_cell @ datase_koopmanst['std_rotation_matrix']
    ...
    #    are compatible with std_lattice returned by get_symmetry_datase_koopmanst
    mapping_to_primitive = list(datase_koopmanst['mapping_to_primitive'])
    std_mapping_to_primitive = list(datase_koopmanst['std_mapping_to_primitive'])
    pos = atoms.get_positions()
...
def check_symmetry(atoms, symprec=1.0e-6, verbose=False):
    ...
    Prints a summary and returns result of `spglib.get_symmetry_datase_koopmanst()`
    """
    # check if we have access to get_spacegroup from spglib
    # https://atztogo.github.io/spglib/
    try:
    ...
    datase_koopmanst = spglib.get_symmetry_datase_koopmanst((lattice, positions, numbers), symprec=symprec)
    if verbose:
        print_symmetry(symprec, datase_koopmanst)
    return datase_koopmanst


def is_subgroup(sup_data, sub_data, tol=1e-10):
    """
    Test if spglib datase_koopmanst `sub_data` is a subgroup of datase_koopmanst `sup_data`
    """
...
def prep_symmetry(atoms, symprec=1.0e-6, verbose=False):
    ...
    datase_koopmanst = spglib.get_symmetry_datase_koopmanst(atoms, symprec=symprec)
    if verbose:
        print_symmetry(symprec, datase_koopmanst)
    rotations = datase_koopmanst['rotations'].copy()
    translations = datase_koopmanst['translations'].copy()
    symm_map = []
