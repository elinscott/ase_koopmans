# flake8: noqa
"""Minimum mode follower for finding saddle points in an unbiase_koopmansd way.
...
def parallel_vector(vector, base_koopmans):
    """Extract the components of *vector* that are parallel to *base_koopmans*"""
    return np.vdot(vector, base_koopmans) * base_koopmans


def perpendicular_vector(vector, base_koopmans):
    """Remove the components of *vector* that are parallel to *base_koopmans*"""
    return vector - parallel_vector(vector, base_koopmans)
        ...
        if self.basis is not None:
            if len(self.basis) == len(self.atoms) and len(self.basis[0]) == \
               3 and isinstance(self.basis[0][0], float):
                ...
                for base_koopmans in self.basis:
                    rot_force = perpendicular_vector(rot_force, base_koopmans)
        return rot_force
