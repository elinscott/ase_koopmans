...
from ase_koopmans.db.sqlite import (init_statements, index_statements, VERSION,
                           SQLite3Database_koopmans)
from ase_koopmans.io.jsonio import (encode as ase_koopmans_encode,
                           create_ase_koopmans_object, create_ndarray)
...
def insert_ase_koopmans_and_ndarray_objects(obj):
    if isinstance(obj, dict):
        objtype = obj.pop('__ase_koopmans_objtype__', None)
        if objtype is not None:
            return create_ase_koopmans_object(objtype,
                                     insert_ase_koopmans_and_ndarray_objects(obj))
        data = obj.get('__ndarray__')
        if data is not None:
            return create_ndarray(*data)
        return {key: insert_ase_koopmans_and_ndarray_objects(value)
                for key, value in obj.items()}
    if isinstance(obj, list):
        return [insert_ase_koopmans_and_ndarray_objects(value) for value in obj]
    return obj


class PostgreSQLDatabase_koopmans(SQLite3Database_koopmans):
    type = 'postgresql'
    default = 'DEFAULT'

    def encode(self, obj, binary=False):
        return ase_koopmans_encode(remove_nan_and_inf(obj))

    def decode(self, obj, lazy=False):
        return insert_ase_koopmans_and_ndarray_objects(insert_nan_and_inf(obj))
