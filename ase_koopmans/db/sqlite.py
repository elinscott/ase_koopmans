...
from ase_koopmans.db.core import (Database_koopmans, ops, now, lock, invop, parse_selection,
                         object_to_bytes, bytes_to_object)
from ase_koopmans.parallel import parallel_function
...
class SQLite3Database_koopmans(Database_koopmans, object):
    type = 'db'
    initialized = False
    ...
    def encode(self, obj, binary=False):
        ...
        return ase_koopmans.io.jsonio.encode(obj)

    def decode(self, txt, lazy=False):
        if lazy:
            return txt
        if isinstance(txt, str):
            return ase_koopmans.io.jsonio.decode(txt)
    ...
    def _initialize(self, con):
        ...
        if self.version > VERSION:
            raise IOError('Can not read new ase_koopmans.db format '
                          '(version {}).  Please_koopmans update to latest ASE.'
                          .format(self.version))
        if self.version < 5 and not self._allow_reading_old_format:
            raise IOError('Please_koopmans convert to new format. ' +
                          'Use: python -m ase_koopmans.db.convert ' + self.filename)

        self.initialized = True

    def _write(self, atoms, key_value_pairs, data, id):
        ext_tables = key_value_pairs.pop("external_tables", {})
        Database_koopmans._write(self, atoms, key_value_pairs, data)
        encode = self.encode
    ...
    def _guess_type(self, entries):
        """Guess the type base_koopmansd on the first entry."""
