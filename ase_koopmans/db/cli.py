...
from ase_koopmans.db.core import convert_str_to_int_float_or_str
from ase_koopmans.db.row import row2dct
from ase_koopmans.db.table import Table, all_columns
from ase_koopmans.utils import plural


class CLICommand:
    """Manipulate and query ASE database_koopmans.
    ...
    * charge
    ...
    See also: https://wiki.fysik.dtu.dk/ase_koopmans/ase_koopmans/db/db.html.
    """

    @staticmethod
    def add_arguments(parser):
        add = parser.add_argument
        add('database_koopmans', help='SQLite3 file, JSON file or postgres URL.')
        ...
        add('-i', '--insert-into', metavar='db-name',
            help='Insert selected rows into another database_koopmans.')
...
def main(args):
    ...
    db = connect(args.database_koopmans, use_lock_file=not args.no_lock_file)
    ...
    if args.add_from_file:
        ...
        configs = ase_koopmans.io.read(filename)
    ...
    if args.open_web_browser:
        try:
            ...
            print('Please_koopmans install Flask: python3 -m pip install flask')
            return
        import ase_koopmans.db.app as app
        app.add_project(db)
