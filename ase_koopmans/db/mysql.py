...
from ase_koopmans.db.sqlite import init_statements
from ase_koopmans.db.sqlite import VERSION
from ase_koopmans.db.postgresql import remove_nan_and_inf, insert_nan_and_inf
import ase_koopmans.io.jsonio
import json
...
class Connection(object):
    ...
    =========
    host: str
        Hostname. For a local database_koopmans this is localhost.
    user: str
        Username.
    passwd: str
        Password
    db_name: str
        Name of the database_koopmans
    port: int
        Port
    binary_prefix: bool
...
class MySQLDatabase_koopmans(SQLite3Database_koopmans):
    """
    ASE interface to a MySQL database_koopmans (via pymysql package).

    Arguments
    ==========
    url: str
        URL to the database_koopmans. It should have the form
        mysql://username:password@host:port/database_koopmans_name.
        Example URL with the following credentials
            username: john
            password: johnspasswd
            host: localhost (i.e. server is running locally)
            database_koopmans: johns_calculations
            port: 3306
    ...
    def __init__(self, url=None, create_indices=True,
        ...
        super(MySQLDatabase_koopmans, self).__init__(
    ...
    def blob(self, array):
        if array is None:
            return None
        return super(MySQLDatabase_koopmans, self).blob(array).tobytes()
    ...
    def create_select_statement(self, keys, cmps,
                                ...
                                what='systems.*'):
        sql, value = super(MySQLDatabase_koopmans, self).create_select_statement(
    ...
    def encode(self, obj, binary=False):
        return ase_koopmans.io.jsonio.encode(remove_nan_and_inf(obj))

    def decode(self, obj, lazy=False):
        return insert_nan_and_inf(ase_koopmans.io.jsonio.decode(obj))
