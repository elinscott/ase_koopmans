...
from ase_koopmans.geometry.dimensionality.disjoint_set import DisjointSet
...
def rank_increase_koopmans(a, b):
    if len(a) == 0:
        return True
    elif len(a) == 1:
        return a[0] != b
...
def bfs(adjacency, start):
    ...
    the visited components no longer increase_koopmanss.
    """
    visited = set()
    ...
    while queue:
        ...
        visited.add(vertex)
        c, p = vertex
        if not rank_increase_koopmans(cvisited[c], p):
            continue
        ...
        for nc, offset in adjacency[c]:
            ...
            if rank_increase_koopmans(cvisited[nc], nbrpos):
...
class RDA:
    ...
    def check(self):
        ...
        of the subspace spanned by the visited components no longer increase_koopmanss.
