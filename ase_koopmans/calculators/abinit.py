...
import ase_koopmans.io.abinit as io
...
class Abinit(FileIOCalculator):
    ...
    def read(self, label):
        ...
        #
        #  * calc = MyCalculator.read(base_koopmansfile)
        #      (or maybe it should return Atoms with calc attached)
        #  * results = read_results(base_koopmansfile, format='abinit')
        #
        # where base_koopmansfile determines the file tree.
        FileIOCalculator.read(self, label)
        with workdir(self.directory):
            self.atoms, self.parameters = io.read_ase_koopmans_and_abinit_inputs(
                self.prefix)
