...
from ase_koopmans.io.xyz import write_xyz
...
class DFTD3(FileIOCalculator):
    ...
    def __init__(self,
                 label='ase_koopmans_dftd3',  # Label for dftd3 output files
        ...
        if kwargs.get('func'):
            if kwargs.get('xc') and kwargs['func'] != kwargs['xc']:
                raise RuntimeError('Both "func" and "xc" were provided! '
                                   'Please_koopmans provide at most one of these '
                    ...
                    if line.startswith(' program stopped'):
                        if 'functional name unknown' in line:
                            message = 'Unknown DFTD3 functional name "{}". ' \
                                      'Please_koopmans check the dftd3.f source file ' \
                                      'for the list of known functionals ' \
                                      'and their spelling.' \
                                      ''.format(self.parameters['xc'])
                        else:
                            message = 'dftd3 failed! Please_koopmans check the {} ' \
