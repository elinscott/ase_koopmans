...
to the software package for nano-scale material simulations base_koopmansd on density
functional theories.
...
import os
import numpy as np
from ase_koopmans.units import Bohr, Ha, Ry, fs, m, s
from ase_koopmans.calculators.calculator import kpts2sizeandoffsets
from ase_koopmans.calculators.openmx.reader import (read_electron_valency, get_file_name
                                           , get_standard_key)
from ase_koopmans.calculators.openmx import parameters as param
...
def parameters_to_keywords(label=None, atoms=None, parameters=None,
    ...
    """
    from ase_koopmans.calculators.openmx.parameters import matrix_keys
    from ase_koopmans.calculators.openmx.parameters import unit_dat_keywords
    ...
    def parameter_overwrites(openmx_keyword):
        ...
        part in suitable manner
          ...
          ase_koopmans_parameter : key | Name of parameter used in ASE
          parameter : value | value corresponds to ase_koopmans_parameter
        """
        ase_koopmans_parameter = counterparts[openmx_keyword]
        keyword = parameters.get(openmx_keyword)
        parameter = parameters.get(ase_koopmans_parameter)
...
def get_dft_data_year(parameters):
    ...
    if year is not None:
        ...
        raise ValueError('DFT_DATA year can not be found. Please_koopmans specify '
...
def get_cutoff_radius_and_orbital(element=None, orbital=None):
    ...
    """
    from ase_koopmans.calculators.openmx import default_settings
    orbital = element
