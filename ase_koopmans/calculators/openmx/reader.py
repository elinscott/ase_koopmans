...
"""
#  from ase_koopmans.calculators import SinglePointDFTCalculator
import os
import struct
import numpy as np
from ase_koopmans.units import Ha, Bohr, Debye


def read_openmx(filename=None, debug=False):
    from ase_koopmans.calculators.openmx import OpenMX
    ...
    the files. Unlike previous version, we read the information base_koopmansd on file.
...
def get_standard_key(key):
    ...
    It is recommended to use lower case_koopmans alphabet letter. Not Upper. Thus, we
...
def get_file_name(extension='.out', filename=None, absolute_directory=True):
    ...
    if absolute_directory:
        ...
        return os.path.base_koopmansname(directory + '/' + prefix + extension)
