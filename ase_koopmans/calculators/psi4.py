...
from ase_koopmans.calculators.calculator import Calculator, all_changes
from ase_koopmans.calculators.calculator import InputError, ReadError
from ase_koopmans.calculators.calculator import CalculatorSetupError
...
class Psi4(Calculator):
    """
    An ase_koopmans calculator for the popular open source Q-chem code
    psi4.
    ...
    def set_psi4(self, atoms=None):
        ...
        if 'kpts' in self.parameters:
            raise InputError('psi4 is a non-periodic code, and thus does not'
                             ' require k-points. Please_koopmans remove this '
                             'argument.')
        ...
        if 'smearing' in self.parameters:
            raise InputError('Finite temperature DFT is not implemented in'
                             ...
                             'cannot be utilized. please_koopmans remove this '
                             'argument')

        if 'xc' in self.parameters:
            raise InputError('psi4 does not accept the `xc` argument please_koopmans'
