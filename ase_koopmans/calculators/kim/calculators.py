...
from ase_koopmans.data import atomic_masses, atomic_numbers
...
def LAMMPSRunCalculator(
    ...
    def get_params(model_name, supported_units, supported_species, atom_style):
        ...
        # In case_koopmans the SM supplied its own atom_style in its model-init -- only needed
...
def ASAPCalculator(model_name, model_type, options, **kwargs):
    ...
    if model_type == "pm":
        ...
        # Verify units (ASAP models are expected to work with "ase_koopmans" units)
        if supported_units != "ase_koopmans":
            raise KIMCalculatorError(
                'KIM Simulator Model units are "{}", but expected to '
                'be "ase_koopmans" for ASAP.'.format(supported_units)
            )
