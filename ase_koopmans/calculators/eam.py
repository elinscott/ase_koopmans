...
``potential``              file of potential in ``.eam``, ``.alloy``, ``.adp`` or ``.fs``
                           ...
                           In case_koopmans of file object the ``form`` argument is required
...
* Although currently not fast, this calculator can be good for trying
  small calculations or for creating new potentials by matching base_koopmansline
...
End EAM Interface Documentation
    ...
    def set_form(self, fileobj):
        """set the form variable base_koopmansd on the file name suffix"""
...
#            r = np.apply_along_axis(np.linalg.norm, 1, rvec)  # sloow
    ...
    def calculate_forces(self, atoms):
        # calculate the forces base_koopmansd on derivatives of the three EAM functions
