...
import re
import numpy as np
from ase_koopmans.units import eV, Ang
            ...
            if m:
                ...
                while(True):
                    ...
                    g = lines[s].split()[3:6]
                     ...
                     # Uncomment the section below to separate the numbers when there is no space between them, in the case_koopmans of long numbers. This prevents the code to break if numbers are too big.
...
class Conditions:
    ...
    def min_distance_rule(self, sym1, sym2,
        ...
        """Find pairs of atoms to label base_koopmansd on proximity.
        ...
        for i1, i2 in zip(index_assigned_sym1, index_assigned_sym2):
            if dist_mat[i1, i2] > max_distance:
                raise ValueError('Cannot unambiguously apply minimum-distance '
                                 ...
                                 'If you wish to ignore this, then increase_koopmans '
                                 'max_distance.')
