...
class Prism:
    ...
    :param cell: cell in ase_koopmans coordinate system
    ...
    **Implementation**
    ...
    - ase_koopmans_cell (the simulated system in the ASE coordination system)
    - lammps_tilt (ase_koopmans-cell rotated to be an lower triangular matrix)
    - lammps_cell (same volume as tilted cell, but reduce edge length)

    The translation between 'ase_koopmans_cell' and 'lammps_cell' is down with a
    ...
    def __init__(self, cell, pbc=(True, True, True), tolerance=1.0e-8):
        ...
        #    rot_mat * lammps_tilt^T = ase_koopmans_cell^T
        # => lammps_tilt * rot_mat^T = ase_koopmans_cell
        # => lammps_tilt             = ase_koopmans_cell * rot_mat
        qtrans, ltrans = np.linalg.qr(cell.T, mode="complete")
        self.rot_mat = qtrans
        self.lammps_tilt = ltrans.T
        self.ase_koopmans_cell = cell
    ...
    def update_cell(self, lammps_cell):
        """Rotate new lammps cell into ase_koopmans coordinate system

        :param lammps_cell: new lammps cell received after executing lammps
        :returns: ase_koopmans cell
        :rtype: np.array
        ...
        new_ase_koopmans_cell = np.dot(self.lammps_tilt, self.rot_mat.T)
        # assuming the cell changes are mostly isotropic
        new_vol = np.linalg.det(new_ase_koopmans_cell)
        old_vol = np.linalg.det(self.ase_koopmans_cell)
        test_residual = self.ase_koopmans_cell.copy()
        test_residual *= (new_vol / old_vol) ** (1.0 / 3.0)
        test_residual -= new_ase_koopmans_cell
        if any(
                np.linalg.norm(test_residual, axis=1)
                > 0.5 * np.linalg.norm(self.ase_koopmans_cell, axis=1)
        ):
            print(
        ...
        return new_ase_koopmans_cell

    def vector_to_lammps(self, vec, wrap=False):
        """Rotate vector from ase_koopmans coordinate system to lammps one

        :param vec: to be rotated ase_koopmans-vector
    ...
    def vector_to_ase_koopmans(self, vec, wrap=False):
        """Rotate vector from lammps coordinate system to ase_koopmans one

        :param vec: to be rotated lammps-vector
        :param wrap: was vector wrapped into 'lammps_cell'
        :returns: ase_koopmans-vector
        :rtype: np.array
