...
from ase_koopmans.calculators.lammps.unitconvert import convert

# "End mark" used to indicate that the calculation is done
CALCULATION_END_MARK = "__end_of_ase_koopmans_invoked_calculation__"
...
def lammps_create_atoms(fileobj, parameters, atoms, prismobj):
    ...
    :type atoms: Atoms
    :param prismobj: coordinate transformation between ase_koopmans and lammps
    :type prismobj: Prism

    """
    if parameters["verbose"]:
        fileobj.write("## Original ase_koopmans cell\n".encode("utf-8"))
        fileobj.write(
            "".join(
                [
                    "# {0:.16} {1:.16} {2:.16}\n".format(*x)
    ...
    if parameters["always_triclinic"] or prismobj.is_skewed():
        fileobj.write(
            "region ase_koopmanscell prism 0.0 {0} 0.0 {1} 0.0 {2} ".format(
                xhi, yhi, zhi
            ).encode("utf-8")
        )
        fileobj.write(
        ...
        fileobj.write(
            "region ase_koopmanscell block 0.0 {0} 0.0 {1} 0.0 {2} "
    ...
    for sym, pos in zip(symbols, atoms.get_positions()):
        ...
        if parameters["verbose"]:
            fileobj.write(
                "# atom pos in ase_koopmans cell: {0:.16} {1:.16} {2:.16}\n"
