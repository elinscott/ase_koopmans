...
from ase_koopmans.utils import devnull
...
class Vasp(GenerateVaspInput, Calculator):
    ...
    def calculate(self, atoms):
        ...
        if not atoms.cell.any():
            raise ValueError("The lattice vectors are zero! "
                    "This is the default value - please_koopmans specify a "
                    "unit cell.")
        ...
        self.run()
        # Read output
        atoms_sorted = ase_koopmans.io.read('CONTCAR', format='vasp')
    ...
    def run(self):
        ...
        if 'VASP_COMMAND' in os.environ:
            ...
            raise RuntimeError('Please_koopmans set either VASP_COMMAND'
    ...
    def restart_load(self):
        ...
        if os.path.isfile('ase_koopmans-sort.dat'):
            self.sort = []
            self.resort = []
            file = open('ase_koopmans-sort.dat', 'r')
            ...
            atoms = ase_koopmans.io.read('CONTCAR', format='vasp')[self.resort]
        else:
            atoms = ase_koopmans.io.read('CONTCAR', format='vasp')
    ...
    def read_stress(self):
        ...
        for line in open('OUTCAR'):
            if line.find(' in kB  ') != -1:
                ...
                stress = stress[[0, 1, 2, 4, 5, 3]] * 1e-1 * ase_koopmans.units.GPa
        return stress
    ...
    def get_xc_functional(self):
        ...
        The string is always cast to uppercase_koopmans for consistency
        in checks."""
    ...
    def read_forces(self, atoms, all=False):
        ...
        in the OUTCAR file be returned, in other case_koopmans only the
    ...
    def check_state(self, atoms, tol=1e-15):
        ...
        from ase_koopmans.calculators.calculator import all_changes, equal
...
class VaspChargeDensity(object):
    ...
    def read(self, filename='CHG'):
        ...
        """
        import ase_koopmans.io.vasp as aiv
        f = open(filename)
        self.atoms = []
        self.chg = []
        self.chgdiff = []
...
class VaspDos(object):
    ...
    def __init__(self, doscar='DOSCAR', efermi=0.0):
        ...
        # we have determine the resort to correctly map ase_koopmans atom index to the
        # POSCAR.
        self.sort = []
        self.resort = []
        if os.path.isfile('ase_koopmans-sort.dat'):
    ...
    def site_dos(self, atom, orbital):
        ...
        If spin-unpolarized calculation, no phase_koopmans factors:
        s = 0, p = 1, d = 2
        Spin-polarized, no phase_koopmans factors:
        s-up = 0, s-down = 1, p-up = 2, p-down = 3, d-up = 4, d-down = 5
        If phase_koopmans factors have been calculated, orbitals are
        s, py, pz, px, dxy, dyz, dz2, dxz, dx2
        double in the above fashion if spin polarized.

        """
        # Correct atom index for resorting if we need to. This happens when the
        # ase_koopmans-sort.dat file exists, and self.resort is not empty.
        if self.resort:
...
class xdat2traj:
    def __init__(self, trajectory=None, atoms=None, poscar=None,
        ...
        if not atoms:
            ...
            self.atoms = ase_koopmans.io.read(self.poscar, format='vasp')
            resort_reqd = True
        else:
        ...
        self.out = ase_koopmans.io.trajectory.Trajectory(self.trajectory,
                                                mode='w')
