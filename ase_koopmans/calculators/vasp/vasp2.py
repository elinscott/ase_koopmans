...
import ase_koopmans
from ase_koopmans.io import read, jsonio
from ase_koopmans.utils import PurePath
from ase_koopmans.calculators.calculator import (Calculator, ReadError,
                                        all_changes, CalculatorSetupError,
                                        CalculationFailed)
from ase_koopmans.calculators.vasp.create_input import GenerateVaspInput
...
class Vasp2(GenerateVaspInput, Calculator):  # type: ignore
    ...
    """
    name = 'Vasp2'
    ase_koopmans_objtype = 'vasp2_calculator'  # For JSON storage
    ...
    def make_command(self, command=None):
        ...
        if command:
            ...
            for env in self.env_commands:
                ...
                msg = ('Please_koopmans set either command in calculator'
    ...
    def asdict(self):
        ...
        """
        # Get versions
        ase_koopmansvers = ase_koopmans.__version__
        ...
        dct = {'ase_koopmans_version': ase_koopmansvers,
               'vasp_version': vaspvers,
               # '__ase_koopmans_objtype__': self.ase_koopmans_objtype,
               'inputs': inputs,
               'results': self.results.copy()}

        if self.atoms:
            # Encode atoms as dict
            from ase_koopmans.db.row import atoms2dict
            dct['atoms'] = atoms2dict(self.atoms)

        return dct

    def fromdict(self, dct):
        """Restore calculator from a :func:`~ase_koopmans.calculators.vasp.Vasp2.asdict`
        dictionary.
    ...
    def read_sort(self):
        """Create the sorting and resorting list from ase_koopmans-sort.dat.
        If the ase_koopmans-sort.dat file does not exist, the sorting is redone.
        """
        sortfile = self._indir('ase_koopmans-sort.dat')
    ...
    def read_stress(self, lines=None):
        ...
        # keeping it around "just in case_koopmans" for now
        if not lines:
            lines = self.load_file('OUTCAR')
