...
class LinearCombinationCalculator(Calculator):
    ...
    def __init__(self, calcs, weights, atoms=None):
        ...
        calcs: list
            List of an arbitrary number of :mod:`ase_koopmans.calculators` objects.
        weights: list of float
            Weights for each calculator in the list.
        atoms: Atoms object
            Optional :class:`~ase_koopmans.Atoms` object to which the calculator will be attached.
        """
        ...
        for calc in calcs:
            if not isinstance(calc, Calculator):
                raise ValueError('All the calculators should be inherited form the ase_koopmans\'s Calculator class')
