...
from ase_koopmans.calculators.calculator import Parameters
...
class DemonNano(FileIOCalculator):
    ...
    def _write_argument(self, key, value, fd):
       ...
       # Has to be in lowercase_koopmans for deMon-nano to work
           line = value.lower()
           fd.write(line)
           fd.write('\n')
...
# Analysis routines
    ...
    def read_energy(self):
       """Read energy from deMon.ase_koopmans output file."""

       epath = pl.Path(self.label)

       if not (epath / 'deMon.ase_koopmans').exists():
           raise ReadError('The deMonNano output file for ASE {0} does not exist'
                           .format(epath))

       filename = self.label + '/deMon.ase_koopmans'
    ...
    def read_forces(self, atoms):
       """Read forces from the deMon.ase_koopmans file."""
    ...
    def deMon_inp_to_atoms(self, filename):
       ...
       # set atoms object
       atoms = ase_koopmans.Atoms(symbols=chem_symbols, positions=xyz)
