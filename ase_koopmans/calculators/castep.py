"""This module defines an interface to CASTEP for
    use by the ASE (Webpage: http://wiki.fysik.dtu.dk/ase_koopmans)
...
from ase_koopmans.calculators.general import Calculator
...
from ase_koopmans.calculators.calculator import kpts2sizeandoffsets
from ase_koopmans.dft.kpoints import BandPath
from ase_koopmans.parallel import paropen
from ase_koopmans.io.castep import read_param
from ase_koopmans.io.castep import read_bands
from ase_koopmans.constraints import FixCartesian
...
*$HOME/.ase_koopmans/* or *ase_koopmans/ase_koopmans/calculators/* within the ASE library. The file should
be generated the first time it is needed, but you can generate a new keywords
file in the currect directory with ``python -m ase_koopmans.calculators.castep``.
...
``find_pspots``            Boolean whether to search for pseudopotentials in
                           ...
                           value.  A RuntimeError will be raised in case_koopmans
...
``_set_atoms``             (``=False``) : setting this to True will overwrite
                           ...
                           wise try to assign forces etc. base_koopmansd on the atom's
...
``.find_pspots(pspot=<library>, suffix=<suffix>)``
  ...
  will be searched for case_koopmans insensitive.  Regular expressions are accepted, and
...
``ase_koopmans.io.castep.read_seed('path-to/seed')``
...
``.set_kpts(kpoints)``
  ...
  with density of at least 10 Ang (base_koopmansd on the unit cell of currently-attached
...
``.band_structure(bandfile=None)``
  Read a band structure from _seedname.bands_ file. This returns an ase_koopmans
...
End CASTEP Interface Documentation
    ...
    def __init__(self, directory='CASTEP', label='castep',
        ...
        # initialize the ase_koopmans.calculators.general calculator
        Calculator.__init__(self)

        from ase_koopmans.io.castep import write_cell
        ...
        for keyword, value in kwargs.items():
            ...
            else:  # the general case_koopmans
    ...
    def set_bandpath(self, bandpath):
        """Set a band structure path from ase_koopmans.dft.kpoints.BandPath object
        ...
        Args:
            bandpath (:obj:`ase_koopmans.dft.kpoints.BandPath` or None):
        ...
        if bandpath is None:
            ...
            raise TypeError('Band structure path must be an '
                            'ase_koopmans.dft.kpoint.BandPath object')
    ...
    def set_kpts(self, kpts):
        ...
        The simplest useful case_koopmans is to give a 3-tuple of integers specifying
        ...
        # Case_koopmans 1: Clear parameters with set_kpts(None)
        if kpts is None:
            clear_mp_keywords()
            pass

        # Case_koopmans 2: list of explicit k-points with weights
        # e.g. [[ 0,    0,   0,    0.125],
        #       [ 0,   -0.5, 0,    0.375],
        #       [-0.5,  0,  -0.5,  0.375],
        #       [-0.5, -0.5, -0.5, 0.125]]
        ...
        # Case_koopmans 3: list of explicit kpts formatted as list of str
        ...
        # Case_koopmans 4: list or tuple of MP samples e.g. [3, 3, 2]
        ...
        # Case_koopmans 5: str representation of Case_koopmans 3 e.g. '3 3 2'
        elif isinstance(kpts, str):
            self.set_kpts([int(x) for x in kpts.split()])

        # Case_koopmans 6: dict of options e.g. {'size': (3, 3, 2), 'gamma': True}
        ...
        # Case_koopmans 7: some other iterator. Try treating as a list:
    ...
    def _castep_find_last_record(self, castep_file):
        ...
        is the case_koopmans, the line number of the last banner is message
    ...
    def read(self, castep_file=None):
        ...
        if castep_file is None:
            ...
            # in this case_koopmans we assume that we have a fileobj already, but check
        ...
        if self._seed is None:
            self._seed = os.path.splitext(os.path.base_koopmansname(castep_file))[0]
        ...
        while True:
            ...
            try:
                # in case_koopmans we need to rewind back one line, we memorize the bit
                ...
                if not line or out.tell() > record_end:
                    ...
                    # TODO: generalize for non-Monkhorst Pack case_koopmans
                    # (i.e. kpoint lists) -
        ...
        if self.atoms and not self._set_atoms:
            ...
            # positions_frac_ase_koopmans = self.atoms.get_scaled_positions(wrap=False)
            ...
            for iase_koopmans in range(n_atoms):
                for icastep in range(n_atoms):
                    if (species[icastep] == self.atoms[iase_koopmans].symbol
                            and not atoms_assigned[icastep]):
                        positions_frac_atoms[iase_koopmans] = \
                            positions_frac_castep[icastep]
                        forces_atoms[iase_koopmans] = np.array(forces_castep[icastep])
                        if iprint > 1 and calculate_hirshfeld:
                            hirsh_atoms[iase_koopmans] = np.array(hirsh_castep[icastep])
                        if spin_polarized:
                            # reordering not necessary in case_koopmans all spins == 0
                            spins_atoms[iase_koopmans] = np.array(spins_castep[icastep])
                        mulliken_charges_atoms[iase_koopmans] = np.array(
            ...
            # This covers the case_koopmans that we simply want to open a .castep file.
            ...
            if self.atoms:
                ...
                constraints = []
            atoms = ase_koopmans.atoms.Atoms(species,
                                    cell=lattice_real,
    ...
    def find_pspots(self, pspot='.+', elems=None,
        ...
        In most case_koopmanss, it will be sufficient to not specify anything, if you
    ...
    def get_property(self, name, atoms=None, allow_calculation=True):
        # High-level getter for compliance with the database_koopmans module...
        # in principle this would not be necessary any longer if we properly
        # base_koopmansd this class on `Calculator`
        if name == 'forces':
    ...
    def get_potential_energy(self, atoms, force_consistent=False):
        # here for compatibility with ase_koopmans/calculators/general.py
    ...
    def prepare_input_files(self, atoms=None, force_write=None):
        ...
        from ase_koopmans.io.castep import write_param
    ...
    def __setattr__(self, attr, value):
        ...
        Value assigment is case_koopmans insensitive!
        """
        ...
        if attr.startswith('_'):
            ...
            if value is not None:
                if attr == 'atoms' and not isinstance(value, ase_koopmans.atoms.Atoms):
                    raise TypeError(
                        '%s is not an instance of ase_koopmans.atoms.Atoms.' % value)
        ...
        if attr not in (list(self.cell._options.keys())
            ...
            if self._kw_tol == 0:
                ...
                warnings.warn('Option "%s" is not known - please_koopmans set any new'
...
class CastepCell(CastepInputFile):
    ...
    def _parse_species_pot(self, value):
        ...
        if hasattr(value, '__getitem__'):
            ...
            if not all(map(lambda x: len(x) == 2, value)):
                print('Please_koopmans specify pseudopotentials in python as')
    ...
    def _parse_positions_abs_intermediate(self, value):
        if not isinstance(value, ase_koopmans.atoms.Atoms):
...
class ConversionError(Exception):
    ...
    def __str__(self):
        return 'Could not convert %s = %s to %s\n' \
            ...
            + 'type or this keyword needs some special care. Please_koopmans feel\n'\
...
def import_castep_keywords(castep_command='',
    ...
    searchpaths = [path,
                   os.path.expanduser('~/.ase_koopmans'),
                   os.path.join(ase_koopmans.__path__[0], 'calculators')]
    try:
...
""")
        ...
        print("""\n\n    Stored %s in %s.
                 Copy it to your
    ASE installation under ase_koopmans/calculators for system-wide installation
""" % (filename, os.path.abspath(path)))
        print("""\n\n    Using a *nix OS this can be a simple as\nmv %s %s""" %
              (os.path.join(os.path.abspath(path),
                            filename),
               os.path.join(os.path.dirname(ase_koopmans.__file__),
                            'calculators')))
...
if __name__ == '__main__':
    ...
    print('For system wide usage, it can be copied into an ase_koopmans installation')
