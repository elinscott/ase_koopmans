...
Please_koopmans read the license file (../../LICENSE)
...
from ase_koopmans.units import Ha, Bohr
from ase_koopmans.io import read, write
from ase_koopmans.calculators.calculator import FileIOCalculator
from ase_koopmans.calculators.calculator import PropertyNotImplementedError, ReadError
...
class Turbomole(FileIOCalculator):
    ...
    def reset(self):
        ...
        ase_koopmans_files = [f for f in os.listdir('.') if f.startswith('ASE.TM.')]
        for f in self.tm_files + self.tm_tmp_files + ase_koopmans_files:
            if os.path.exists(f):
                os.remove(f)
    ...
    def read_forces(self):
        ...
        if iline < 0:
            raise RuntimeError('Please_koopmans check TURBOMOLE gradients')
