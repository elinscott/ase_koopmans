...
def closest_distances_generator(atom_numbers, ratio_of_covalent_radii):
    ...
    The distances are base_koopmansd on the covalent radii of the atoms.
    """
    cr = covalent_radii
...
def db_call_with_error_tol(db_cursor, expression, args=[]):
    """In case_koopmans the GA is used on older versions of networking
    ...
    raise sqlite3.OperationalError(
        'Database_koopmans still locked after 10 attempts (20 s)')


def save_trajectory(confid, trajectory, folder):
    """Saves traj files to the database_koopmans folder.
...
def get_rdf(atoms, rmax, nbins, distance_matrix=None,
    ...
    This may or may not be appropriate in case_koopmanss where one or more
...
class CellBounds:
    ...
    >>> from ase_koopmans.ga.utilities import CellBounds
