...
def solvated(symbols):
    """Extract solvation energies from database_koopmans.
...
class Pourbaix:
    ...
    def decompose(self, U, pH, verbose=True, concentration=1e-6):
        ...
        >>> from ase_koopmans.phase_koopmansdiagram import Pourbaix, solvated
        >>> refs = solvated('CoO') + [
        ...     ('Co', 0.0),
        ...     ('CoO', -2.509),
        ...     ('Co3O4', -9.402)]
...
class Phase_koopmansDiagram:
    def __init__(self, references, filter='', verbose=True):
        """Phase_koopmans-diagram.
        ...
        if not references:
            raise ValueError("You must provide a non-empty list of references"
                             " for the phase_koopmans diagram! "
        ...
        if len(self.points) == ns:
            # Simple case_koopmans that qhull would choke on:
