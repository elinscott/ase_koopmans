...
import ase_koopmans
import ase_koopmans.lattice.hexagonal
from ase_koopmans.build import bulk, molecule

from ase_koopmans.neighborlist import (mic, neighbor_list, primitive_neighbor_list,
                              first_neighbors)
...
def test_neighbor_kernel():
    ...
    # two atoms
    a = ase_koopmans.Atoms('CC', positions=[[0.5, 0.5, 0.5], [1,1,1]], cell=[10, 10, 10],
                  pbc=True)
    ...
    for pbc in [True, False, [True, False, True]]:
        a = ase_koopmans.Atoms('4001C', cell=[29, 29, 29])
    ...
    # test_small_cell
    a = ase_koopmans.Atoms('C', positions=[[0.5, 0.5, 0.5]], cell=[1, 1, 1],
                  pbc=True)
    ...
    a = ase_koopmans.Atoms('CC', positions=[[0.5, 0.5, 0.5],
                                   [1.1, 0.5, 0.5]],
    ...
    a = ase_koopmans.Atoms('CC', positions=[[9.5, 0.5, 0.5],
                                   [10.1, 0.5, 0.5]],
    ...
    # test_hexagonal_cell
    for sx in range(3):
        a = ase_koopmans.lattice.hexagonal.Graphite('C', latticeconstant=(2.5, 10.0),
                                           size=[sx+1,sx+1,1])
    ...
    # test pbc
    nat = 10
    atoms = ase_koopmans.Atoms(numbers=range(nat),
                      cell=[(0.2, 1.2, 1.4),
                            (1.4, 0.1, 1.6),
                            (1.3, 2.0, -0.1)])
    ...
    # Empty atoms object
    i, D, d, j, S = neighbor_list("iDdjS", ase_koopmans.Atoms(), 1.0)
    ...
    # argument.
    i = neighbor_list("i", ase_koopmans.Atoms(), 1.0)
    assert i.dtype == np.int
    assert i.shape == (0,)
