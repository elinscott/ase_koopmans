...
from ase_koopmans.calculators.gaussian import Gaussian, GaussianOptimizer, GaussianIRC
...
def test_optimizer(atoms):
    ...
    opt_ase_koopmans = LBFGS(atoms, trajectory='ase_koopmans_opt.traj')
    opt_ase_koopmans.run(fmax=1e-2)
    e_ase_koopmansopt = atoms.get_potential_energy()
    assert e_gaussopt - e_ase_koopmansopt == pytest.approx(0., abs=1e-3)
