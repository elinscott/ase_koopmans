import pytest
from ase_koopmans.io.cif import read_cif
...
_publ_section_title
;Crystal structures of the low-temperature quartz-type phase_koopmanss of SiO2 and GeO2 at elevated pressure
...
def test_pycodcif():
    ...
    with open('myfile.cif') as temp:
        cif_ase_koopmans = read_cif(temp, 0, reader='ase_koopmans')
        cif_pycodcif = read_cif(temp, 0, reader='pycodcif')

        assert [repr(x) for x in cif_ase_koopmans] == [repr(x) for x in cif_pycodcif]
