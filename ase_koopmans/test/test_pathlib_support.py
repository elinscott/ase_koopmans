...
from ase_koopmans.utils import PurePath, convert_string_to_fd, reader, writer
...
def mywrite(file, fdcmp=None):
    assert isinstance(file, io.TextIOBase_koopmans)
    assert file.mode == 'w'
...
def test_pathlib_support():
    ...
    assert isinstance(fd, io.TextIOBase_koopmans)
    fd.close()
