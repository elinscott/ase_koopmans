...
from ase_koopmans.utils import workdir
from ase_koopmans.test.factories import (Factories, CalculatorInputs,
                                make_factory_fixture, get_testing_executables)


@pytest.fixture(scope='session')
def enabled_calculators():
    from ase_koopmans.calculators.calculator import names as calculator_names
...
def monkeypatch_disabled_calculators(request, enabled_calculators):
    from ase_koopmans.test.testsuite import disable_calculators, test_calculator_names
    from ase_koopmans.calculators.calculator import names as calculator_names
...
def datafiles():
    try:
        import ase_koopmanstest
    except ImportError:
        return {}
    else:
        return ase_koopmanstest.datafiles.paths
...
def pytest_generate_tests(metafunc):
    from ase_koopmans.test.factories import parametrize_calculator_tests
...
class CLI:
    def ase_koopmans(self, args):
        if isinstance(args, str):
            import shlex
            args = shlex.split(args)

        proc = Popen(['ase_koopmans', '-T'] + args,
    ...
    def shell(self, command, calculator_name=None):
        from ase_koopmans.test.testsuite import runshellcommand
        runshellcommand(command, calculator_name=calculator_name)


@pytest.fixture(scope='session')
def datadir():
    from ase_koopmans.test.testsuite import datadir
    return datadir
