...
from ase_koopmans.data import atomic_numbers, covalent_radii
from ase_koopmans.geometry import complete_cell, find_mic, wrap_positions
from ase_koopmans.geometry import minkowski_reduce
from ase_koopmans.cell import Cell


def natural_cutoffs(atoms, mult=1, **kwargs):
    """Generate a radial cutoff for every atom base_koopmansd on covalent radii.
    ...
    atoms length list of radii base_koopmansd on this idea.
...
def build_neighbor_list(atoms, cutoffs=None, **kwargs):
    ...
    atoms : :class:`~ase_koopmans.Atoms` object
        Atoms to build Neighborlist for.
    cutoffs: list of floats
        Radii for each atom. If not given it will be produced by calling :func:`ase_koopmans.neighborlist.natural_cutoffs`
    kwargs: arbitrary number of options
        Will be passed to the constructor of :class:`~ase_koopmans.neighborlist.NeighborList`

    Returns:

    return: :class:`~ase_koopmans.neighborlist.NeighborList`
        A :class:`~ase_koopmans.neighborlist.NeighborList` instance (updated).
    """
    if cutoffs is None:
...
def get_distance_indices(distanceMatrix, distance):
    ...
    distanceMatrix: any one of scipy.sparse matrices (NxN)
        ...
        :func:`~ase_koopmans.neighborlist.get_distance_matrix`
    distance: integer
    ...
    distances longer than one, we need to add the lower values for case_koopmanss
    ...
    cutoff: float or dict
        Cutoff for neighbor search. It can be:
            ...
            * A list/array with a per atom value: This specifies the radius of
              ...
              within each others neighborhood. See :func:`~ase_koopmans.neighborlist.natural_cutoffs`
...
def get_connectivity_matrix(nl, sparse=True):
    ...
    >>> from ase_koopmans import neighborlist
...
class NewPrimitiveNeighborList:
    ...
    skin: float
        ...
        last call to the :meth:`~ase_koopmans.neighborlist.NewPrimitiveNeighborList.update()`
    ...
    def build(self, pbc, cell, coordinates):
        ...
        if self.bothways:
            ...
            for a in range(natoms):
                ...
                # Force correct type and shape for case_koopmans of no neighbors:
...
class NeighborList:
    ...
    cutoffs: list of float
        List of cutoff radii - one for each atom. If the spheres (defined by
        their cutoff radii) of two atoms overlap, they will be counted as
        neighbors. See :func:`~ase_koopmans.neighborlist.natural_cutoffs` for an example on how to
        get such a list.

    skin: float
        If no atom has moved more than the skin-distance since the
        last call to the :meth:`~ase_koopmans.neighborlist.NeighborList.update()` method,
    ...
    bothways: bool
        Return all neighbors.  Default is to return only "half" of
        the neighbors.
    primitive: :class:`~ase_koopmans.neighborlist.PrimitiveNeighborList` or :class:`~ase_koopmans.neighborlist.NewPrimitiveNeighborList` class
        Define which implementation to use. Older and quadratically-scaling
        :class:`~ase_koopmans.neighborlist.PrimitiveNeighborList` or newer and
        linearly-scaling :class:`~ase_koopmans.neighborlist.NewPrimitiveNeighborList`.
    ...
    def update(self, atoms):
        """
        See :meth:`ase_koopmans.neighborlist.PrimitiveNeighborList.update` or
        :meth:`ase_koopmans.neighborlist.PrimitiveNeighborList.update`.
        """
    ...
    def get_neighbors(self, a):
        """
        See :meth:`ase_koopmans.neighborlist.PrimitiveNeighborList.get_neighbors` or
        :meth:`ase_koopmans.neighborlist.PrimitiveNeighborList.get_neighbors`.
        """
    ...
    def get_connectivity_matrix(self, sparse=True):
        """
        See :func:`~ase_koopmans.neighborlist.get_connectivity_matrix`.
        """
