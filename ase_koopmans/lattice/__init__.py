...
from ase_koopmans.build.bulk import bulk as newbulk
from ase_koopmans.dft.kpoints import parse_path_string, sc_special_points, BandPath
from ase_koopmans.utils import pbc2pbc


@functools.wraps(newbulk)
def bulk(*args, **kwargs):
    warnings.warn('Use ase_koopmans.build.bulk() instead', stacklevel=2)
...
class BravaisLattice(ABC):
    ...
    >>> from ase_koopmans.lattice import FCC, MCL
    >>> FCC.name
    'FCC'
    >>> FCC.longname
    'face-centred cubic'
    ...
    def tocell(self):
        """Return this lattice as a :class:`~ase_koopmans.cell.Cell` object."""
    ...
    def cellpar(self):
        ...
        See :func:`ase_koopmans.geometry.Cell.cellpar`."""
    ...
    def bandpath(self, path=None, npoints=None, special_points=None,
        ...
        """Return a :class:`~ase_koopmans.dft.kpoints.BandPath` for this lattice.

        See :meth:`ase_koopmans.cell.Cell.bandpath` for description of parameters.
...
special_points={GNPSS1XYY1Z}, kpts=[51x3])
        ...
        .. note:: This produces the standard band path following AFlow
           ...
           you will need :meth:`ase_koopmans.cell.Cell.bandpath` instead or
        ...
        if path is None:
            ...
            from ase_koopmans.dft.kpoints import resolve_custom_points
...
@bravaisclass('base_koopmans-centred orthorhombic', 'orthorhombic', 'orthorhombic',
              'oC', 'abc',
...
@bravaisclass('base_koopmans-centred monoclinic', 'monoclinic', 'monoclinic', 'mC',
              ('a', 'b', 'c', 'alpha'),
...
class MCLC(BravaisLattice):
    ...
    def _variant_name(self, a, b, c, alpha):
        # from ase_koopmans.geometry.cell import mclc
...
If you don't care, please_koopmans use Cell.fromcellpar() instead."""
...
def identify_lattice(cell, eps=2e-4, *, pbc=True):
    ...
    from ase_koopmans.geometry.bravais_type_engine import niggli_op_table
    ...
    for latname in LatticeChecker.check_order:
        ...
        # lattices, at least for MCL.  In that case_koopmans we will pick the
...
class LatticeChecker:
    ...
    def _bct_orci_lengths(self):
        ...
        #                     == b² / 2 (ORCI)
        #   a3 · a3 + a1 · a2 == c² / 2
        # We use these to get a, b, and c in those case_koopmanss.
        prods = self.prods
...
def get_2d_bravais_lattice(origcell, eps=2e-4, *, pbc=True):
    ...
    for op in ops:
        ...
        # Maximum one angle different from 90 deg in 2d please_koopmans
