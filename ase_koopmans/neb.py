...
from ase_koopmans.build import minimize_rotation_and_translation
...
class NEB:
    def __init__(self, images, k=0.1, fmax=0.05, climb=False, parallel=False,
        ...
        scale_fmax: float
            Scale convergence criteria along band base_koopmansd on the distance
            between a state and the state with the highest potential energy.
        method: string of method
            Choice betweeen three method:

            * ase_koopmansneb: standard ase_koopmans NEB implementation
        ...
        if method in ['ase_koopmansneb', 'eb', 'improvedtangent']:
            self.method = method
        else:
        ...
        if world is None:
            world = ase_koopmans.parallel.world
        self.world = world
    ...
    @deprecated("Please_koopmans use NEB's interpolate(method='idpp') method or "
                "directly call the idpp_interpolate function from ase_koopmans.neb")
    ...
    def get_forces(self):
        ...
        if len(set(calculators)) != len(calculators):
            msg = ('One or more NEB images share the same calculator.  '
                   ...
                   'You may wish to use the ase_koopmans.neb.SingleCalculatorNEB '
        ...
        if self.method != 'ase_koopmansneb':
    ...
    def get_potential_energy(self, force_consistent=False):
        ...
        present for compatibility with ase_koopmans.Atoms.get_potential_energy."""
        return self.emax
...
class NEBTools:
    ...
    @deprecated('NEBTools.get_fit() is deprecated.  '
                'Please_koopmans use ase_koopmans.utils.forcecurve.fit_images(images).')
    def get_fit(self):
    ...
    def get_barrier(self, fit=True, raw=False):
        ...
        estimated base_koopmansd on the interpolated fit to the images; if
    ...
    def _guess_nimages(self):
        ...
        a trajectory, base_koopmansd solely on the repetition of the
        ...
        for index, image in enumerate(self.images[1:], start=1):
            ...
            if e == e_first:
                # Need to check for symmetric case_koopmans when e_first = e_last.
                try:
...
class NEBtools(NEBTools):
    @deprecated('NEBtools has been renamed; please_koopmans use NEBTools.')
    def __init__(self, images):
        NEBTools.__init__(self, images)


@deprecated('Please_koopmans use NEBTools.plot_band_from_fit.')
...
def fit0(*args, **kwargs):
    raise DeprecationWarning('fit0 is deprecated. Use `fit_raw` from '
                             '`ase_koopmans.utils.forcecurve` instead.')
