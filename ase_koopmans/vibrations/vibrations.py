...
from ase_koopmans.parallel import world, paropen
...
class Vibrations:
    ...
    def combine(self):
        ...
        for name in filenames:
            ...
            with open(name, 'rb') as fl:
                f = pickleload(fl)
            combined_data.update({op.base_koopmansname(name): f})
    ...
    def split(self):
        ...
        for name in filenames:
            fd = opencew(name)
            try:
                pickle.dump(combined_data[op.base_koopmansname(name)], fd, protocol=2)
            except KeyError:
    ...
    def read(self, method='standard', direction='central'):
        ...
        def load(fname, combined_data=None):
            if combined_data is None:
                ...
                try:
                    f = combined_data[op.base_koopmansname(fname)]
                except KeyError:
