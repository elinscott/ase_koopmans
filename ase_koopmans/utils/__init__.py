...
from ase_koopmans.formula import formula_hill, formula_metal

__all__ = ['exec_', 'base_koopmansstring', 'import_module', 'seterr', 'plural',
...
def convert_string_to_fd(name, world=None):
    ...
    if name == '-':
        return sys.stdout
    if isinstance(name, (base_koopmansstring, PurePath)):
...
def search_current_git_hash(arg, world=None):
    ...
    # in case_koopmans this is just symlinked into $PYTHONPATH
...
def iofunction(func, mode):
    ...
    def iofunc(file, *args, **kwargs):
        openandclose = isinstance(file, (base_koopmansstring, PurePath))
        fd = None
        try:
            if openandclose:
...
# in ase_koopmans.io.jsonio, but we'd rather import them from a 'basic' module
# like ase_koopmans/utils than one which triggers a lot of extra (cyclic) imports.

def write_json(self, fd):
    """Write to JSON file."""
    from ase_koopmans.io.jsonio import write_json as _write_json
    _write_json(fd, self)
...
def read_json(cls, fd):
    ...
    from ase_koopmans.io.jsonio import read_json as _read_json
    obj = _read_json(fd)
...
def jsonable(name):
    ...
    Pokes JSON-base_koopmansd read and write functions into the class.
    ...
    defines a string called ase_koopmans_objtype, the decoder will want to convert
    the object back into its original type when reading."""
    def jsonableclass(cls):
        cls.ase_koopmans_objtype = name
...
def warn_legacy(feature_name):
    warnings.warn(
        ...
        'know whether it works or how to use it.  Please_koopmans rehabilitate it '
