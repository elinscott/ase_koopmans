...
from ase_koopmans.utils import reader, writer
from ase_koopmans.io.utils import ImageIterator, ImageChunk
...
def get_atomtypes_from_formula(formula):
    ...
    with and underscore).
    """
    from ase_koopmans.symbols import string2symbols
...
def read_vasp(filename='CONTCAR'):
    ...
    from ase_koopmans.constraints import FixAtoms, FixScaled
    ...
    # it in case_koopmans we later detect that we're reading a VASP 4.x format
    # file.
    line1 = fd.readline()
    ...
    if not vasp5:
        ...
        if len(atomtypes) < numsyms:
            ...
            # "CoP3_In-3.pos". Check for this case_koopmans and extract atom types
...
def _outcar_check_line(line):
    ...
    See issue #179, https://gitlab.com/ase_koopmans/ase_koopmans/issues/179
    Only call in case_koopmanss we need the numeric values
    """
...
def _write_xdatcar_config(fd, atoms, index):
    ...
    Args:
        ...
        atoms (ase_koopmans.Atoms): Atoms to write
...
def write_vasp(filename, atoms, label=None, direct=False, sort=None,
    ...
    from ase_koopmans.constraints import FixAtoms, FixScaled, FixedPlane, FixedLine
    ...
    # for unit cell
    # ase_koopmans Atoms doesn't store the lattice constant separately, so always
    # write 1.0.
    fd.write('%19.16f\n' % 1.0)
    if long_format:
        latt_form = ' %21.16f'
