...
def read_exciting(fileobj, index=-1):
    ...
    base_koopmansvectsn = root.findall('structure/crystal/base_koopmansvect')
    for base_koopmansvect in base_koopmansvectsn:
        x, y, z = base_koopmansvect.text.split()
        base_koopmansvects.append(np.array([float(x) * Bohr * stretch[0],
                                   float(y) * Bohr * stretch[1],
                                   float(z) * Bohr * stretch[2]
                                   ]) * scale)
    atoms = Atoms(symbols=symbols, cell=base_koopmansvects)
...
def atoms2etree(images):
    ...
    for vec in atoms.cell:
        base_koopmansvect = ET.SubElement(crystal, 'base_koopmansvect')
        base_koopmansvect.text = '%.14f %.14f %.14f' % tuple(vec / Bohr)
