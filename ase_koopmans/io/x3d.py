...
def write_x3d(filename, atoms, format=None):
    ...
    Args:
        ...
        format - str, either 'X3DOM' for web-browser compatibility or 'X3D'
            to be readable by Blender. `None` to detect format base_koopmansd on file
...
class X3D:
    ...
    def write(self, filename, datatype=None):
        """Writes output to either an 'X3D' or an 'X3DOM' file, base_koopmansd on
        ...
        if datatype == 'X3DOM':
            ...
            w(2, ' href="https://www.x3dom.org/x3dom/release_koopmans/x3dom.css">')
            w(2, '</link>')
            w(2, '<script type="text/javascript"')
            w(2, ' src="https://www.x3dom.org/x3dom/release_koopmans/x3dom.js">')
            w(2, '</script>')
            w(1, '</head>')
            w(1, '<body>')
            w(2, '<X3D>')
