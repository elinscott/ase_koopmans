...
This module implements the read(), iread() and write() functions in ase_koopmans.io.
...
The xyz format is implemented in the ase_koopmans/io/xyz.py file which has a
...
class IOFormat:
    ...
    @property
    def module(self):
        if not self.module_name.startswith('ase_koopmans.io.'):
            raise ValueError('Will only import modules from ase_koopmans.io, '
...
def define_io_format(name, desc, code, *, module=None, ext=None,
    ...
    fmt = IOFormat(name, desc, code, module_name='ase_koopmans.io.' + module,
                   encoding=encoding)
