...
# (CODATA1986 for ase_koopmans-3.6.0.2515 vs CODATA2002 for CASTEP 5.01).
#
...
units_ase_koopmans = {
    ...
    'kB': ase_koopmans.units.kB,
    'a0': ase_koopmans.units.Bohr,
    't0': ase_koopmans.units._hbar * ase_koopmans.units.J / ase_koopmans.units.Hartree,
    'c': ase_koopmans.units._c,
    'me': ase_koopmans.units._me / ase_koopmans.units._amu,
    'Pascal': 1.0 / ase_koopmans.units.Pascal}
...
def write_castep_cell(fd, atoms, positions_frac=False, force_write=False,
    ...
    # one
    from ase_koopmans.calculators.castep import Castep, CastepCell
    ...
    if len(constraints):
        ...
        for constr in constraints:
            if not isinstance(constr, _supported_constraints):
                ...
                print('         not supported by the CASTEP ase_koopmans interface')
                break
...
def read_freeform(fd):
    ...
    from ase_koopmans.calculators.castep import CastepInputFile
...
def read_castep_cell(fd, index=None, calculator_args={}, find_spg=False,
    ...
    from ase_koopmans.calculators.castep import Castep
    ...
    atoms = ase_koopmans.Atoms(**aargs)
    ...
    if find_spg:
        ...
        if spglib is not None:
            symmd = spglib.get_symmetry_datase_koopmanst(atoms)
        ...
        if len(value) == 3:
            ...
            if np.linalg.norm(direction) == 0:
                print('Error: Found linearly dependent constraints attached '
                      'to atoms %s' % (absolute_nr))
                continue
            constraint = ase_koopmans.constraints.FixedLine(
                a=absolute_nr,
                direction=direction)
            constraints.append(constraint)
        elif len(value) == 1:
            constraint = ase_koopmans.constraints.FixedPlane(
                a=absolute_nr,
    ...
    if fixed_atoms:
        constraints.append(
            ase_koopmans.constraints.FixAtoms(indices=sorted(fixed_atoms)))
    if constraints:
...
def read_castep_castep(fd, index=None):
    ...
    convenience to comply with the generic "read()" in ase_koopmans.io

    Please_koopmans note that this routine will return an atom ordering as found
...
def read_castep_castep_old(fd, index=None):
    """
    DEPRECATED
    Now replaced by ase_koopmans.calculators.castep.Castep.read(). Left in for future
...
def read_castep_geom(fd, index=None, units=units_CODATA2002):
    ...
    for i, line in enumerate(txt):
        ...
        if line.find('<-- R') > 0 and start_found:
            ...
            image = ase_koopmans.Atoms(species, geom, cell=cell, pbc=True)
...
def read_castep_phonon(fd, index=None, read_vib_data=False,
    ...
    # ase_koopmans.vibrations.Vibrations.modes):
...
def write_param(filename, param, check_checkfile=False,
    ...
    if os.path.isfile(filename) and not force_write:
        print('ase_koopmans.io.castep.write_param: Set optional argument')
    ...
    if interface_options is not None:
        ...
        out.write('# by ase_koopmans.io.castep.read_seed()\n')
...
def read_seed(seed, new_seed=None, ignore_internal_keys=False):
    ...
    Please_koopmans note that this routine will return an atoms ordering as specified
    in the cell file! It will thus undo the potential reordering internally
    done by castep.
    """

    directory = os.path.abspath(os.path.dirname(seed))
    seed = os.path.base_koopmansname(seed)
    ...
    if os.path.isfile(castepfile):
        ...
        if os.path.isfile(checkfile):
            atoms.calc._check_file = os.path.base_koopmansname(checkfile)

        # sync the top-level object with the
        # one attached to the calculator
        atoms = atoms.calc.atoms
    else:
        # There are case_koopmanss where we only want to restore a calculator/atoms
        # setting without a castep file...
        pass
        # No print statement required in these case_koopmanss
