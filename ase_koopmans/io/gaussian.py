...
from ase_koopmans.calculators.calculator import InputError
...
def write_gaussian_in(fd, atoms, properties=None, **params):
    ...
    if method.lower() in _problem_methods:
        warnings.warn(
            ...
            'Please_koopmans use these methods with caution.'.format(method)
        )
    ...
    # unit cell vectors, in case_koopmans of periodic boundary conditions
    ...
    if basisfile is not None:
        ...
        if basis is not None and basis.lower() == 'gen':
            raise InputError('Please_koopmans set basisfile')
...
def _compare_merge_configs(configs, new):
    ...
    In those case_koopmanss, rather than repeating the structure in the list of
...
def read_gaussian_out(fd, index=-1):
    ...
    for line in fd:
        ...
        if (line == 'Input orientation:'
            ...
            # in this case_koopmans, because it might be rotated relative to the input
