...
from ase_koopmans.quaternions import Quaternions
...
def lammps_data_to_ase_koopmans_atoms(
    data,
    colnames,
    cell,
    celldisp,
    ...
    :param atomsobj: function to create ase_koopmans-Atoms object
    :param order: sort atoms by id. Might be faster to turn off
    :param specorder: list of species to map lammps types to ase_koopmans-species
    (usually .dump files to not contain type to species mapping)
    :param prismobj: Coordinate transformation between lammps and ase_koopmans
    :type prismobj: Prism
    :param units: lammps units for unit transformation between lammps and ase_koopmans
    ...
    if prismobj:
        celldisp = prismobj.vector_to_ase_koopmans(celldisp)
    ...
    if quaternions:
        ...
        if prismobj:
            positions = prismobj.vector_to_ase_koopmans(positions, wrap=True)
    ...
    if velocities is not None:
        if prismobj:
            velocities = prismobj.vector_to_ase_koopmans(velocities)
        out_atoms.set_velocities(velocities)
    if charges is not None:
        out_atoms.set_initial_charges(charges)
    if forces is not None:
        if prismobj:
            forces = prismobj.vector_to_ase_koopmans(forces)
...
def construct_cell(diagdisp, offdiag):
    ...
    # create ase_koopmans-cell from lammps-box
...
def read_lammps_dump_text(fileobj, index=-1, **kwargs):
    ...
    while len(lines) > n_atoms:
        ...
        if "ITEM: BOX BOUNDS" in line:
            # save labels behind "ITEM: BOX BOUNDS" in triclinic case_koopmans
            # (>=lammps-7Jul09)
            ...
            # determine cell tilt (triclinic case_koopmans!)
            if len(celldata[0]) > 2:
        ...
        if "ITEM: ATOMS" in line:
            ...
            out_atoms = lammps_data_to_ase_koopmans_atoms(
                data=data,
                colnames=colnames,
                cell=cell,
                celldisp=celldisp,
...
def read_lammps_dump_binary(
    ...
    while True:
        try:
            ...
            # periodic case_koopmans: b 0 = 'p'
            # non-peridic case_koopmanss 1: 'f', 2 : 's', 3: 'm'
            ...
            # map data-chunk to ase_koopmans atoms
            out_atoms = lammps_data_to_ase_koopmans_atoms(
                data=data,
                colnames=colnames,
                cell=cell,
