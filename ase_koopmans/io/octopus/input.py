...
special_ase_koopmans_keywords = set(['kpts'])
...
def process_special_kwargs(atoms, kwargs):
    ...
    for kw in special_ase_koopmans_keywords:
...
def normalize_keywords(kwargs):
    """Reduce keywords to unambiguous form (lowercase_koopmans)."""
    newkwargs = {}
...
def kwargs2cell(kwargs):
    ...
    if boxshape_is_ase_koopmans_compatible(kwargs):
...
def boxshape_is_ase_koopmans_compatible(kwargs):
...
def kwargs2atoms(kwargs, directory=None):
    ...
    example when running 'ase_koopmans gui somedir/inp'."""
    kwargs = normalize_keywords(kwargs)
    ...
    def get_positions_from_block(keyword):
        ...
        if types:
            ...
            for sym, tag in types.items():
                ase_koopmans_types[('X', tag)] = sym
            info = {'types': ase_koopmans_types}  # 'info' dict for Atoms object
        else:
            tags = None
            info = None
    ...
    if pdbcoords is not None:
        ...
        pbc = atoms.pbc
        adjust_positions_by_half_cell = True
        # Due to an error in ASE pdb, we can only test the nonperiodic case_koopmans.
        # atoms.cell *= Bohr # XXX cell?  Not in nonperiodic case_koopmans...
        atoms.positions *= Bohr
        if sum(atoms.pbc) != 0:
    ...
    if cell is None:
        ...
        if cell is not None and atoms is not None:
            atoms.cell = cell
        # In case_koopmans of boxshape = sphere and similar, we still do not have
        # a cell.
    ...
    # got its BCs from reading the file.  In the latter case_koopmans
...
def generate_input(atoms, kwargs):
    ...
    use_ase_koopmans_cell = (boxshape == 'parallelepiped')
    atomskwargs = atoms2kwargs(atoms, use_ase_koopmans_cell)

    if use_ase_koopmans_cell:
...
def atoms2kwargs(atoms, use_ase_koopmans_cell):
    kwargs = {}
...
def read_octopus_in(fd, get_kwargs=False):
    ...
    # failing 'ase_koopmans gui somedir/inp'
    try:
        fname = fd.name
