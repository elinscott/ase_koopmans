...
def read_wannier90_in(fd):
    ...
    Base_koopmansd on ase_koopmans.io.castep.read_freeform
    """
...
def _path_lengths(path: str, cell: Cell, bands_point_num: int) -> List[int]:
    # Construct the metric for reciprocal space (base_koopmansd off Wannier90's "utility_metric" subroutine)
    recip_lat = 2 * math.pi * cell.reciprocal().array
    recip_metric = recip_lat @ recip_lat.T

    # Work out the lengths Wannier90 will assign each path (base_koopmansd off Wannier90's "plot_interpolate_bands" subroutine)
    kpath_pts: List[int] = []
...
def num_wann_lookup(proj):
    ...
    if proj in database_koopmans:
        ...
        l, mr = proj.split(',')
        if l in database_koopmans and '=' in mr:
            _, val = mr.split('=')
...
def num_wann_from_projections(projections: List[Dict[str, Any]], atoms: Atoms):
    # Works out the value of 'num_wann' base_koopmansd on the 'projections' block
    num_wann = 0
