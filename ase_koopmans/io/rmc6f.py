...
def _read_line_of_atoms_section(fields):
    ...
    properties: list[str|float]
        List of Atoms properties base_koopmansd on rmc6f style.
    ...
    # Get style used for rmc6f from file base_koopmansd on number of columns
    ncols = len(fields)
...
def _read_process_rmc6f_lines_to_pos_and_cell(lines):
    ...
    pos : dict{int:list[str|float]}
        Dict for each atom id and Atoms properties base_koopmansd on rmc6f style.
        Basically, have 1) element and fractional coordinates for 'labels'
        or 'no_labels' style and 2) same for 'magnetic' style except adds
        the spin.
        Examples for 1) 'labels' or 'no_labels' styles or 2) 'magnetic' style:
...
def write_rmc6f(filename, atoms, order=None, atom_type_map=None):
    ...
    header_lines = [
        ...
        "(Generated by ASE - Atomic Simulation Environment https://wiki.fysik.dtu.dk/ase_koopmans/ )",  # noqa: E501
    ...
    # get formatting for writing output base_koopmansd on atom arrays
