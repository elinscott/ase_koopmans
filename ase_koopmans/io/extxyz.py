...
from ase_koopmans.calculators.calculator import all_properties, Calculator
from ase_koopmans.calculators.singlepoint import SinglePointCalculator
from ase_koopmans.spacegroup.spacegroup import Spacegroup
from ase_koopmans.parallel import paropen
from ase_koopmans.constraints import FixAtoms, FixCartesian
from ase_koopmans.io.formats import index2range
        ...
        if key.lower() not in UNPROCESSED_KEYS:
            ...
            try:
                ...
                if len(numvalue) == 1:
                    ...
                    # special case_koopmans: 3x3 matrix, fortran ordering
...
def parse_properties(prop_str):
    ...
    for name, ptype, cols in zip(fields[::3],
        ...
        ase_koopmans_name = REV_PROPERTY_NAME_MAP.get(name, name)
        ...
        properties[name] = (ase_koopmans_name, cols)
...
def _read_xyz_frame(lines, natoms, properties_parser=key_val_str_to_dict,
    ...
    for ln in range(natoms):
        try:
            ...
            raise XYZError('ase_koopmans.io.extxyz: Frame has {} atoms, expected {}'
    ...
    if nvec > 0:
        for ln in range(nvec):
            try:
                ...
                raise XYZError('ase_koopmans.io.adfxyz: Frame has {} cell vectors, '
    ...
    arrays = {}
    for name in names:
        ase_koopmans_name, cols = properties[name]
        if cols == 1:
            value = data[name]
        else:
            value = np.vstack([data[name + str(c)]
                               for c in range(cols)]).T
        arrays[ase_koopmans_name] = value
    ...
    for key in list(atoms.info.keys()):
        if key in all_properties:
            ...
            # special case_koopmans for stress- convert to Voigt 6-element form
...
def read_xyz(fileobj, index=-1, properties_parser=key_val_str_to_dict):
    ...
    deal with most use case_koopmanss, ``extxyz.key_val_str_to_dict_regex`` is slightly
    ...
    :attr:`ase_koopmans.atoms.Atoms.arrays` entries named
    `positions`, `numbers`, `masses` and `charges` respectively.

    Additional key-value pairs in the comment line are parsed into the
    :attr:`ase_koopmans.Atoms.atoms.info` dictionary, with the following conventions

     - Values can be quoted with `""`, `''`, `[]` or `{}` (the latter are
       included to ease_koopmans command-line usage as the `{}` are not treated
    ...
    while True:
        ...
        try:
            ...
            raise XYZError('ase_koopmans.io.extxyz: Expected xyz header but got: {}'
                           .format(err))
        ...
        while True:
            ...
            if line.lstrip().startswith('VEC'):
                nvec += 1
                if nvec > 3:
                    raise XYZError('ase_koopmans.io.extxyz: More than 3 VECX entries')
            else:
            ...
            if (calculator is not None
                ...
                for key in all_properties:
                    ...
                    if (isinstance(value, np.ndarray) and len(value.shape) >= 1
                        ...
                        # special case_koopmans for stress, which should be converted
...
# create aliase_koopmanss for read/write functions
