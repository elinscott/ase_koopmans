...
class Element(list):
    ...
    def symbol(self):
        Z = self.Z
        return None if Z is None else ase_koopmans.data.chemical_symbols[Z]
    ...
    def _get(self):
        ...
        if txt.isdigit():
            txt = int(txt)
            try:
                txt = ase_koopmans.data.chemical_symbols[txt]
            except KeyError:
                self.error()
                return None

        if txt in ase_koopmans.data.atomic_numbers:
            return Atoms(txt)
