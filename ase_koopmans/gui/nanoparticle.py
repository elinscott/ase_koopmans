...
# Delayed imports:
# ase_koopmans.cluster.data

from ase_koopmans.cluster.cubic import FaceCenteredCubic, BodyCenteredCubic, SimpleCubic
from ase_koopmans.cluster.hexagonal import HexagonalClosedPacked, Graphite
from ase_koopmans.cluster import wulff_construction
from ase_koopmans.gui.widgets import Element, pybutton


introtext = _("""\
Create a nanoparticle either by specifying the number of layers, or using the
Wulff construction.  Please_koopmans press the [Help] button for instructions on how to
...
2) Choose if you want to specify the number of layers in each direction, or if
   you want to use the Wulff construction.  In the latter case_koopmans, you must
...
# OPTIONAL: Cast to ase_koopmans.Atoms object, discarding extra information:
# atoms = ase_koopmans.Atoms(atoms)
"""
...
class SetupNanoparticle:
    ...
    import_names = {
        'fcc': 'from ase_koopmans.cluster.cubic import FaceCenteredCubic',
        'bcc': 'from ase_koopmans.cluster.cubic import BodyCenteredCubic',
        'sc': 'from ase_koopmans.cluster.cubic import SimpleCubic',
        'hcp': 'from ase_koopmans.cluster.hexagonal import HexagonalClosedPacked',
        'graphite': 'from ase_koopmans.cluster.hexagonal import Graphite'}
    ...
    def set_structure_data(self, *args):
        ...
        if z is None:
            return
        ref = ase_koopmans.data.reference_states[z]
        if ref is None:
            structure = None
        else:
