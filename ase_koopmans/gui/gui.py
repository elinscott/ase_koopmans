...
from ase_koopmans.gui.save import save_dialog
from ase_koopmans.gui.settings import Settings
from ase_koopmans.gui.status import Status
from ase_koopmans.gui.surfaceslab import SetupSurfaceSlab
from ase_koopmans.gui.view import View
...
class GUI(View, Status):
    ...
    def constraints_window(self):
        from ase_koopmans.gui.constraints import Constraints
    ...
    def modify_atoms(self, key=None):
        from ase_koopmans.gui.modify import ModifyAtoms
        ModifyAtoms(self)

    def add_atoms(self, key=None):
        from ase_koopmans.gui.add import AddAtoms
        return AddAtoms(self)

    def cell_editor(self, key=None):
        from ase_koopmans.gui.celleditor import CellEditor
        return CellEditor(self)

    def quick_info_window(self, key=None):
        from ase_koopmans.gui.quickinfo import info
    ...
    def new(self, key=None):
        subprocess.Popen([sys.executable, '-m', 'ase_koopmans', 'gui'])
    ...
    def external_viewer(self, name):
        ...
        fd, filename = tempfile.mkstemp('.xyz', 'ase_koopmans.gui-')
        os.close(fd)
        ...
        return [
            ...
            (_('_Help'),
             [M(_('_About'), partial(ui.about, 'ASE-GUI',
                                     ...
                                     'ase_koopmans/ase_koopmans/gui/gui.html')),
              M(_('Webpage ...'), webpage)])]
...
def webpage():
    import webbrowser
    webbrowser.open('https://wiki.fysik.dtu.dk/ase_koopmans/ase_koopmans/gui/gui.html')
