...
from ase_koopmans.data import reference_states, atomic_numbers, chemical_symbols
...
def bulk(name, crystalstructure=None, a=None, b=None, c=None, *, alpha=None,
    ...
    if natoms != natoms0:
        raise ValueError('Please_koopmans specify {} for {} and not {}'
    ...
    if crystalstructure == 'sc':
        ...
        from ase_koopmans.lattice import BCT
        if basis is None:
            basis = ref.get('basis')
...
def _build_rhl(name, a, alpha, basis):
    from ase_koopmans.lattice import RHL
    lat = RHL(a, alpha)
    cell = lat.tocell()
    if basis is None:
