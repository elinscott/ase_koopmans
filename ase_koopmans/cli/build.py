...
from ase_koopmans.data import ground_state_magnetic_moments
...
class CLICommand:
    ...
    Atom:
        ...
        ase_koopmans build <chemical symbol> ...

    Molecule:

        ase_koopmans build <formula> ...

    where <formula> must be one of the formulas known to ASE
    (see here: https://wiki.fysik.dtu.dk/ase_koopmans/ase_koopmans/build/build.html#molecules).

    Bulk:

        ase_koopmans build -x <crystal structure> <formula> ...

    Examples:

        ase_koopmans build Li  # lithium atom
        ase_koopmans build Li -M 1  # ... with a magnetic moment of 1
        ase_koopmans build Li -M 1 -V 3.5 # ... in a 7x7x7 Ang cell
        ase_koopmans build H2O  # water molecule
        ase_koopmans build -x fcc Cu -a 3.6  # FCC copper
    """
    ...
    def run(args, parser):
        if args.vacuum0:
            parser.error('Please_koopmans use -V or --vacuum instead!')
