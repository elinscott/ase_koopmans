...
from ase_koopmans.utils import search_current_git_hash
from ase_koopmans.io.formats import filetype, ioformats, UnknownFileTypeError
from ase_koopmans.io.ulm import print_ulm_info
from ase_koopmans.io.bundletrajectory import print_bundletrajectory_info
...
class CLICommand:
    ...
    def run(args):
        if not args.filename:
            ...
            if args.calculators:
                print()
                from ase_koopmans.calculators.autodetect import (detect_calculators,
                                                        format_configs)
...
def print_info():
    ...
    for name in ['ase_koopmans', 'numpy', 'scipy', 'ase_koopmans_ext', 'spglib']:
        try:
            module = import_module(name)
        except ImportError:
            if name != 'ase_koopmans_ext':
