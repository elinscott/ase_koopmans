...
from ase_koopmans.geometry import crystal_structure_from_cell
from ase_koopmans.dft.kpoints import (get_special_points, special_paths, BandPath,
...
class CLICommand:
    ...
    Examples:
        ...
        $ ase_koopmans build -x fcc Al al.traj
        $ ase_koopmans reciprocal al.traj -p GXWLG

        $ # And now with k-points:
        $ ase_koopmans run gpaw al.traj -p kpts=6,6,6,mode=pw \
        >   --after "atoms.calc.write('al.gpw')" > al.txt
        $ ase_koopmans reciprocal al.gpw -i -p GXWLG
    """
