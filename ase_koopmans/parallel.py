...
class DummyMPI:
    ...
    def _returnval(self, a, root=-1):
        # MPI interface works either on numbers, in which case_koopmans a number is
...
class MPI:
    ...
    * a dummy implementation for serial runs
    ...
    def __getattr__(self, name):
        ...
        # https://gitlab.com/ase_koopmans/ase_koopmans/-/merge_requests/2695
...
class ParallelModuleWrapper:
    def __getattr__(self, name):
        if name == 'rank' or name == 'size':
            warnings.warn('ase_koopmans.parallel.{name} has been deprecated.  '
                          'Please_koopmans use ase_koopmans.parallel.world.{name} instead.'
                          .format(name=name),
                          FutureWarning)
...
_parallel = sys.modules['ase_koopmans.parallel']
sys.modules['ase_koopmans.parallel'] = ParallelModuleWrapper()  # type: ignore
