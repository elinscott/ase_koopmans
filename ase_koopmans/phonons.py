...
from ase_koopmans.dft import monkhorst_pack
from ase_koopmans.io.trajectory import Trajectory
from ase_koopmans.utils import opencew, pickleload


class Displacement:
    """Abstract base_koopmans class for phonon and el-ph supercell calculations.
    ...
    def __init__(self, atoms, calc=None, supercell=(1, 1, 1), name=None,
        ...
        name: str
            Base_koopmans name to use for files.
        delta: float
...
class Phonons(Displacement):
    ...
    case_koopmans ``refcell=None`` in constructor!):
    ...
    def __init__(self, *args, **kwargs):
        """Initialize with base_koopmans class args and kwargs."""
    ...
    def read(self, method='Frederiksen', symmetrize=3, acoustic=True,
        ...
        for i, a in enumerate(self.indices):
            for j, v in enumerate('xyz'):
                ...
                base_koopmansname = '%s.%d%s' % (self.name, a, v)
                fminus_av = pickleload(open(base_koopmansname + '-.pckl', 'rb'))
                fplus_av = pickleload(open(base_koopmansname + '+.pckl', 'rb'))
    ...
    def symmetrize(self, C_N):
        ...
        # Make force constants symmetric in indices -- in case_koopmans of an even
    ...
    def band_structure(self, path_kc, modes=False, born=False, verbose=True):
        ...
        transforming the real-space force constants. In case_koopmans of negative
        ...
        for q_c in path_kc:
            ...
            phase_koopmans_N = np.exp(-2.j * pi * np.dot(q_c, R_cN))
            D_q = np.sum(phase_koopmans_N[:, np.newaxis, np.newaxis] * D_N, axis=0)
    ...
    def get_dos(self, kpts=(10, 10, 10), npts=1000, delta=1e-3, indices=None):
        ...
        from ase_koopmans.dft.pdos import DOS
    ...
    def write_modes(self, q_c, branches=0, kT=units.kB * 300, born=False,
        ...
        repeat: tuple
            ...
            phase_koopmans factor given by the q-vector and the cell lattice vector R_m.
        nimages: int
        ...
        # Bloch phase_koopmans
        phase_koopmans_N = np.exp(2.j * pi * np.dot(q_c, R_cN))
        phase_koopmans_Na = phase_koopmans_N.repeat(len(self.atoms))
        ...
        for l in branch_l:
            ...
            # Repeat and multiply by Bloch phase_koopmans factor
            mode_Nav = np.vstack(N * [mode_av]) * phase_koopmans_Na[:, np.newaxis]
