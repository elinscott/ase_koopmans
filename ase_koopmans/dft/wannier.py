...
from ase_koopmans.dft.kpoints import get_monkhorst_pack_size_and_offset
from ase_koopmans.transport.tools import dagger, normalize
...
class Wannier:
    ...
    def __init__(self, nwannier, calc,
        ...
        Optional arguments:
          ...
          """
        # Bloch phase_koopmans sign convention.
        # May require special case_koopmanss depending on which code is used.
        sign = -1
    ...
    def get_centers(self, scaled=False):
        ...
        ::
          ...
          pos =  L / 2pi * phase_koopmans(diag(Z))
        """
    ...
    def translate_all_to_cell(self, cell=[0, 0, 0]):
        ...
        **k**-point calculation the periodicity of the orbitals are
        ...
        this case_koopmans it is useful to move the orbitals away from the
    ...
    def get_hopping(self, R):
        ...
        for k, kpt_c in enumerate(self.kpt_kc):
            phase_koopmans = np.exp(-2.j * pi * np.dot(np.array(R), kpt_c))
            H_ww += self.get_hamiltonian(k) * phase_koopmans
        return H_ww / self.Nk
        ...
        for n1 in range(-N1, N1 + 1):
            for n2 in range(-N2, N2 + 1):
                for n3 in range(-N3, N3 + 1):
                    ...
                    phase_koopmans = np.exp(+2.j * pi * np.dot(R, kpt_c))
                    Hk += hop_ww * phase_koopmans
        return Hk
    ...
    def write_cube(self, index, fname, repeat=None, real=True):
        ...
        if real:
            ...
            phase_koopmans_fname = fname.split('.')
            phase_koopmans_fname.insert(1, 'phase_koopmans')
            phase_koopmans_fname = '.'.join(phase_koopmans_fname)
            write(phase_koopmans_fname, atoms, data=np.angle(func), format='cube')
            func = abs(func)
