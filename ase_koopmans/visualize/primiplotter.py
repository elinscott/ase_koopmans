...
from ase_koopmans.utils import warn_legacy
from ase_koopmans.visualize.colortable import color_table
import ase_koopmans.data


class PrimiPlotterBase_koopmans:
    "Base_koopmans class for PrimiPlotter and Povrayplotter."
    ...
    def _getradii(self):
        ...
        try:
            ...
            try:
                ...
                return ase_koopmans.data.covalent_radii[z]
...
class PrimiPlotter(PrimiPlotterBase_koopmans):
    """Primitive PostScript-base_koopmansd plots during a simulation.
    ...
    In case_koopmanss 1a and 3a above, the keys of the dictionary are integers,
    and the values are either numbers (grayscales) or 3-vectors (RGB
    values), or strings with X11 color names, which are then
    translated to RGB values.  Only in case_koopmans 1a and 3a are strings
    ...
    used.  In that case_koopmans the filename is expected to expand to a real
...
class ParallelPrimiPlotter(PrimiPlotter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mpi = ase_koopmans.parallel.world
        if self.mpi is None:
...
class _PostScriptDevice:
    """PostScript base_koopmansd output device."""
