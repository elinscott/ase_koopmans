...
from ase_koopmans.optimize.gpmin.prior import ZeroPrior


class GaussianProcess():
    """Gaussian Process Regression
    It is recommended to be used with other Priors and Kernels from
    ase_koopmans.optimize.gpmin

    Parameters:

    prior: Prior class, as in ase_koopmans.optimize.gpmin.prior
           Defaults to ZeroPrior

    kernel: Kernel function for the regression, as in
            ase_koopmans.optimize.gpmin.kernel
    ...
    def train(self, X, Y, noise=None):
        ...
        Y: targets (i.e. energy and forces). numpy array with
            shape (nsamples, D+1)
        noise: Noise parameter in the case_koopmans it needs to be restated.
        """
