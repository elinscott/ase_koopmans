import os
import numpy as np
from ase_koopmans import io, units
from ase_koopmans.optimize import QuasiNewton
from ase_koopmans.parallel import paropen, world
from ase_koopmans.md import VelocityVerlet
from ase_koopmans.md import MDLogger
...
class MinimaHopping:
        ...
        while True:
            if (totalsteps and self._counter >= totalsteps):
                self._log('msg', 'Run terminated. Step #%i reached of '
                          '%i allowed. Increase_koopmans totalsteps if resuming.'
    ...
    def _resume(self):
        """Attempt to resume a run, base_koopmansd on information in the log
        ...
        been tested in those case_koopmanss currently."""
    ...
    def _check_results(self):
        """Adjusts parameters and positions base_koopmansd on outputs."""
