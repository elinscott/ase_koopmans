...
import ase_koopmans.optimize
from ase_koopmans.calculators.emt import EMT
from ase_koopmans.io import Trajectory


all_optimizers = ase_koopmans.optimize.__all__ + ['PreconLBFGS', 'PreconFIRE',
                                         'SciPyFminCG', 'SciPyFminBFGS']
all_optimizers.remove('QuasiNewton')


def get_optimizer(name):
    # types: (str) -> ase_koopmans.optimize.Optimizer
    if name.startswith('Precon'):
        import ase_koopmans.optimize.precon as precon
        return getattr(precon, name)
    if name.startswith('SciPy'):
        import ase_koopmans.optimize.sciopt as sciopt
        return getattr(sciopt, name)
    return getattr(ase_koopmans.optimize, name)
...
def main():
    ...
    systems = [(row.name, row.toatoms())
               for row in ase_koopmans.db.connect(args.systems).select()]

    db = ase_koopmans.db.connect('results.db')
