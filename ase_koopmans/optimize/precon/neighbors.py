...
from ase_koopmans.neighborlist import neighbor_list
...
def get_neighbours(atoms, r_cut, self_interaction=False):
    ...
    Uses ase_koopmans.neighborlist.neighbour_list to compute neighbors.

    Args:
        atoms: ase_koopmans.atoms object to calculate neighbours for
...
def estimate_nearest_neighbour_distance(atoms):
    ...
    # have a neighbour we increase_koopmans the cutoff and try again, until our
    ...
    while r_cut < 2.0 * max(extent):
        ...
        raise RuntimeError('increase_koopmansd r_cut to twice system extent without '
