"""
Implementation of the Precon abstract base_koopmans class and subclasses
"""
...
from ase_koopmans.constraints import Filter, FixAtoms
from ase_koopmans.utils import longsum
from ase_koopmans.geometry import wrap_positions
import ase_koopmans.utils.ff as ff
import ase_koopmans.units as units
from ase_koopmans.optimize.precon.neighbors import (get_neighbours,
...
class Precon(object):
    ...
    def __init__(self, r_cut=None, r_NN=None,
        ...
        """Initialise a preconditioner object base_koopmansd on passed parameters.
        ...
        Args:
            ...
            mu_c: float
                ...
                if None.
            estimate_mu_eigmode:
                If True, estimates mu base_koopmansd on the lowest eigenmodes of
                unstabilised preconditioner. If False it uses the sine base_koopmansd
                approach.
            ...
            recalc_mu: if True, the value of mu will be recalculated every time
                ...
                case_koopmanss with recalc_mu argument in self.make_precon. If recalc_mu
    ...
    def make_precon(self, atoms, recalc_mu=None):
        """Create a preconditioner matrix base_koopmansd on the passed set of atoms.

        Creates a general-purpose preconditioner for use with optimization
        algorithms, base_koopmansd on examining distances between pairs of atoms in the
    ...
    def _make_sparse_precon(self, atoms, initial_assembly=False,
                            force_stab=False):
        """Create a sparse preconditioner matrix base_koopmansd on the passed atoms.
        ...
        if self.estimate_mu_eigmode:
            ...
            if any(eigvals[0:3] > 1e-6):
                ...
                raise ValueError('Fourth smallest eigenvalue of '
                                 ...
                                 'is too small, increase_koopmans r_cut.')
