# flake8: noqa
"""
The following contains a database_koopmans of 24 gas-phase_koopmans reaction barrier heights for small molecules.
...
def create_dbh24_system(name, **kwargs):
    ...
    """
    if name not in data:
        raise NotImplementedError('System %s not in database_koopmans.' % name)
    d = data[name]
...
def get_dbh24_magmoms(name):
    ...
    """
    if name not in data:
        raise KeyError('System %s not in database_koopmans.' % name)
    else:
