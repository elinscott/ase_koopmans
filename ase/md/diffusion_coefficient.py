...
class DiffCoeff:
	def __init__(self, file_name, timestep, steps_between_saved_images, no_of_segments=1, ignore_n_images=0, continuous=True, molecule_index=None):
		...
		self.timesteps = list(range(0,self.total_images*self.time_between_images,self.time_between_images))
	...
	def plot(self):
		...
		if self.molecule_index == None:
			...
			print('---')
			print('Mean Diffusion Coefficient (X, Y and Z) = %.10f cm^2/s, %.10f m^2/s; Standard Deviation = %.10f cm^2/s, %.10f m^2/s' % (np.mean(self.slopes)*(0.1), np.mean(self.slopes)*(10**-5), np.std(self.slopes)*(0.1), np.std(self.slopes)*(10**-5)))
			print('---')
		...
		#plt.yticks([]) # Does not displays y-ticks
