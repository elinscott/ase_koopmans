...
def lammps_create_atoms(fileobj, parameters, atoms, prismobj):
    ...
    if parameters['always_triclinic'] or prismobj.is_skewed():
        fileobj.write('region asecell prism 0.0 {0} 0.0 {1} 0.0 {2} '.format(
            xhi, yhi, zhi).encode('utf-8'))
        fileobj.write('{0} {1} {2} side in units box\n'.format(
            txy, txz, tyz).encode('utf-8'))
    else:
        fileobj.write('region asecell block 0.0 {0} 0.0 {1} 0.0 {2} '
                      'side in units box\n'.format(xhi, yhi, zhi
                                                   ).encode('utf-8'))
...
def write_lammps_in(lammps_in, parameters, atoms, prismobj,
    ...
    if ('pair_style' in parameters) and ('pair_coeff' in parameters):
        ...
        fileobj.write('pair_style lj/cut 2.5 \n'
                      'pair_coeff * * 1 1 \n'
                      'mass * 1.0 \n'.encode('utf-8'))
    ...
    fileobj.write('print "{0}" \n'.format(
        CALCULATION_END_MARK).encode('utf-8'))
